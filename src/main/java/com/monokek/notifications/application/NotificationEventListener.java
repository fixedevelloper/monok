...
/**
 * Translates {@code ordering}'s (and, for USB print jobs, {@code printing}'s)
 * domain events into named SSE broadcasts — the direct replacement for the
 * frontend's old Echo channels: {@code orders} (order.created/order.updated)
 * and {@code kitchen.station.{id}} (ticket.created, now filtered client-side
 * on {@code stationId} in the payload instead of a server-side per-station
 * channel). {@code .round.added}/{@code .station.updated} had no backend
 * equivalent before this module and aren't ported — the pages that used them
 * already react to {@code order.updated}/{@code ticket.created}.
 */
@Component
public class NotificationEventListener {
    ...
    @ApplicationModuleListener
    void on(PrintJobQueuedEvent event) {
        registry.broadcastToBranch(event.branchId(), "print-job-queued", Map.of(
                "jobId", event.jobId(),
                "printerId", event.printerId(),
                "printerName", event.printerName() == null ? "" : event.printerName(),
                "osPrinterName", event.osPrinterName() == null ? "" : event.osPrinterName(),
