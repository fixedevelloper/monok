...
public interface ProductStockReceiver {
     ...
     * ProductStockMovement} — a no-op if the product doesn't track stock ({@code
     * Product#isTrackStock()} false), so callers never need to check that themselves. Clamps at
